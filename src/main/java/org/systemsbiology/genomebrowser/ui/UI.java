...
public class UI {
    ...
    private void createUi() {
        try {
            ...
            if (MacOsx.isOSX()) MacOsx.createApplicationListener(this);
    ...
    public void zoomToSelection() {
        ...
        if (segment != null) setViewSegment(segment);
    }
    ...
    public void zoomIn() { viewParameters.zoomIn(); }
    public void zoomOut() { viewParameters.zoomOut(); }
    ...
    public void findNext() { gotoAndSelectFeature(app.search.getNext()); }

    // ---- bookmarks ---------------------------------------------------------

    public void bookmarkCurrentSelection(boolean direct) {
        if (viewParameters.getSequence() == null ||
            viewParameters.getSequence() == Sequence.NULL_SEQUENCE) return;
        ...
        Bookmark newBookmark = (segment == null) ? 
            new Bookmark(viewParameters.getSequence().getSeqId(),
                         viewParameters.getStart(),
                         viewParameters.getEnd()) :
            new Bookmark(segment.seqId,
        ...
        if (app.getSequenceFetcher()!=null) { 
            seq = app.getSequenceFetcher().getSequence(newBookmark.getSeqId(), newBookmark.getStrand(), newBookmark.getStart(), newBookmark.getEnd());
        }
        ...
        if (direct) {
            ...
            ds.add(newBookmark);
        }	else showBookmarksDialog(newBookmark);
    }
    ...
    public void showBookmarksDialog(Bookmark bookmark) {
        ...
        if (TranscriptBoundaryPlugin.TRANSCRIPT_TYPE.equals(ds.getAttributes().getString("type"))) {
            TranscriptBoundaryPlugin transcriptBoundaryPlugin = (TranscriptBoundaryPlugin)
                app.getPlugin("TranscriptBoundaryPlugin");
    ...
    public void newBookmarkSet() { mainWindow.bookmarksPanel.newSet(); }
    ...
    public void showProjectPropertiesDialog() {
        ProjectPropertiesDialog dialog = new ProjectPropertiesDialog(mainWindow,
                                                                     app.dataset(),
                                                                     app.options.datasetUrl);
        dialog.setVisible(true);
    }
    ...
    public void showInUcscGenomeBrowser() {
        ...
        if (dataset != null) {
            ...
            if (!isNullOrEmpty(db) && !isNullOrEmpty(domain)) {
                ...
                String url = String.format("%scgi-bin/hgTracks?db=%s&position=%s:%d-%d",
                                           urlBase, db, seq, start, end);
                openBrowser(url);
            } else {
    ...
    public void showImportTrackWizard() {
        ...
        WizardMainWindow wizardWindow = new WizardMainWindow(this.mainWindow, wiz);
    ...
    public void showDeleteTrackDialog() {
        ...
        dialog.addDialogListener(new DialogListener() {
                public void cancel() { log.info("cancel"); }
                public void error(String message, Exception e) {
    ...
    public void showTrackEditor(UUID uuid) {
        ...
        TrackVisualPropertiesEditor trackEditor =
            new TrackVisualPropertiesEditor(app.trackManager, mainWindow, uuid);
    ...
    public void importCoordinateMap() {
        LoadCoordinateMapDialog dialog = new LoadCoordinateMapDialog(mainWindow,
                                                                     app.options);
        dialog.addDialogListener(new DialogListener() {
    ...
    public void trackAdded(UUID trackUuid) {
        ...
        if (showConfirmDialog("Move viewport to location of new data?",
            ...
            if (segments.size() > 0) centerOnSegment(segments.get(0));
        }
    }
