...
public class PeptideBlock implements Block<PeptideFeature> {
    ...
    public PeptideBlock(BlockKey key, int[] starts, int[] ends, String[] names,
                        String[] commonNames, double[] scores) {
        this(key, starts, ends, names, commonNames, scores, null);
    }

    public PeptideBlock(BlockKey key, int[] starts, int[] ends, String[] names,
                        String[] commonNames, double[] scores, int[] redundancy) {
        this.key = key;
        this.starts = starts;
        this.ends = ends;
        this.names = names;
    ...
    public Strand getStrand() {	return key.getStrand();	}
    public Iteratable<PeptideFeature> features() { return new FeaturesIteratable();	}
    public Iteratable<PeptideFeature> features(int start, int end) {
        return new WindowedFeaturesIteratable(start, end);
    }
    public Iterator<PeptideFeature> iterator() { return features(); }
    ...
    class FeaturesIteratable implements Iteratable<PeptideFeature> {
        ...
        public Iterator<PeptideFeature> iterator() { return this;	}		
    }
    ...
    class FlyweightFeature implements PeptideFeature {
        ...
        public double getScore() { return scores[i]; }
        public int getCentralPosition() {	return MathUtils.average(starts[i], ends[i]);	}
        public int getEnd() {	return ends[i];	}
        public String getSeqId() { return key.getSeqId();	}
        public int getStart() { return starts[i];	}
        public Strand getStrand() { return key.getStrand(); }
        public String toString() {
            return String.format("(Feature: %s, %s, %d, %d, %s, %s, %.2f)",
                                 getSeqId(), getStrand(), starts[i], ends[i],
                                 names[i], commonNames[i], scores[i]); 
        }
        public String getLabel() {
            return isNullOrEmpty(getCommonName()) ? getName() : getCommonName();
        }
        public String getCommonName() { return commonNames[i]; }
        public GeneFeatureType getType() { return GeneFeatureType.peptide; }
        public String getName() {	return names[i]; }
        public int getRedundancy() {
            if (redundancy == null)	return -1;
            else return redundancy[i];
        }
    }
}
