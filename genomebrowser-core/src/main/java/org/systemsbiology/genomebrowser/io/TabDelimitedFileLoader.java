...
public class TabDelimitedFileLoader {
     ...
     */
    public void addDoubleColumn(int index) { columns.add(new DoubleColumn(index)); }

    /**
     * Configure the loader to record integer values from the specified column in the file.
     * @param index a zero based index of the column in the tab delimited file.
     */
    public void addFloatColumn(int index) { columns.add(new FloatColumn(index)); }

    /**
     * Configure the loader to record integer values from the specified column in the file.
     * @param index a zero based index of the column in the tab delimited file.
     */
    public void addStringColumn(int index) { columns.add(new StringColumn(index)); }

    /**
     * Configure the loader to record integer values from the specified column in the file.
     * @param index a zero based index of the column in the tab delimited file.
     */
    public void addIntColumn(int index) { columns.add(new IntColumn(index)); }
    ...
    public int getIndexOfColumn(String name) {
        for (int i = 0; i < columnHeaders.length; i++) {
            if (name.equals(columnHeaders[i])) return i;
        }
        return -1;
    }
    ...
    public void loadData(Reader reader) throws IOException {
        ...
        // allocate arrays
        for (Column column : columns) column.allocateArray(size);
        ...
        try {
            ...
            while (line != null) {
                ...
                i++;
                if (i % 1000 == 0) fireIncrementProgressEvent(1);
                line = r.readLine();
            }
            log.info("read " + i + " lines.");
        } finally {
            try { if (r != null) r.close(); } catch (Exception e) { log.error(e); }
        }
    }
    ...
    static class DoubleColumn extends Column {
        ...
        public void allocateArray(int size) { array = new double[size]; }
        public void processLine(int i, String[] fields) {
    ...
    static class FloatColumn extends Column {
        ...
        public void allocateArray(int size) { array = new float[size]; }
        public void processLine(int i, String[] fields) {
    ...
    static class StringColumn extends Column {
        ...
        public void allocateArray(int size) { array = new String[size]; }
        public void processLine(int i, String[] fields) { array[i] = fields[index]; }
    }
    ...
    static class IntAverageColumn extends ComputedColumn {
        ...
        public void allocateArray(int size) { array = new int[size]; }
